...
@app.route('/stream', methods=['GET']) #Go to /stream to see result of scroll
def generate():
    response_headers = {
        ...
        if type(input[key]) == dict:
            if (input[key]['array'][0] != ""):
                ...
                for field in input[key]['array']:
                    ...
                    # if key == 'SIMID':
                    #     field = "*" + field
                    field_string = field_string + str(key) + ".value.keyword:" + str(field)
                    triggerOR = True
    ...
    # indexpattern = "/devicehistory-fromsnapshot-" + datepattern + "*/_search"
    # indexpattern = "/" + indexes + "/_search"
    # indexpattern = "devicehistory-fromsnapshot-20231205"
